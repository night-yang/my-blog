  ...
  <div class="popWrap" v-bind:class="{popWrapActive : this.$store.state.rightSidebarPop}">
    <div>
      <div v-on:click="Close" class="popWrapClose"><i></i></div>
        <h2>{{articleT}}</h2>
      <ul>
        <li v-for='rspdata in article' :key="rspdata.index">
...
export default {
  ...
  computed: {
    article() {
      let dataArr = this.$store.state.rightSidebarData[
        this.$store.state.RightSidebarPopData
      ];
      return dataArr;
    },
    articleT() {
